...
cap g pf04=0
g bc_seguro=0
replace bc_seguro=(bc_pg11p+bc_pg12p+bc_pg14p+bc_pg15p+bc_pg16p+bc_pg17p+bc_pg71p+bc_pg73p+bc_pg71o+bc_pg73o)*bc_ipc if bc_pobp==2 & pf04==7 & (bc_pg101==0 | bc_pg101!=.) // & anio<2001
recode bc_seguro .=0
*-------------------------------------------------------------------------------
*Ingresos laborales
...
cap drop bc_otros_labh
egen bc_otros_labh=sum(bc_otros_lab) if bc_pe4!=7, by(bc_correlat)
cap drop bc_otros_lab2h
egen bc_otros_lab2h=sum(bc_otros_lab2) if bc_pe4!=7, by(bc_correlat)

mvencode bc_as_privados  bc_as_publicos  bc_cpropiasl bc_cpropiacl bc_patrones  bc_cooperat bc_otros_lab bc_otros_lab2 bc_as_otros ot_agropec /*
*/bc_as_agropec cp_agropec pat_agropec, mv(0) override
...
gen bc_ing_lab=0
replace bc_ing_lab= bc_as_privados+ bc_as_publicos+ bc_patrones+ bc_cpropiasl+ bc_cpropiacl+ bc_cooperat +bc_as_otros+bc_as_agropec+cp_agropec+ /* 
*/ pat_agropec+ot_agropec+(bc_otros_lab+bc_otros_lab2)*bc_ipc
...
replace bc_ot_utilidades	=bc_ot_utilidades*bc_ipc // A nivel hogar
...
* esto no estaba en el programa original. ////este benef_ocup_secund, el anio>1990 le pasa por arriba a la línea anterior (AM)
cap g bc_pg23o=0
...
** acá hicismos cambios. eliminamos la variable otros bc_beneficios. 
cap drop bc_beneficios
gen bc_beneficios=ingre_ciud+bc_bs_sociales
cap drop beneficiosh
*gen beneficiosh=ingre_ciudh+bs_socialesh+ bs_fonasah
/*La variable fon_hog_fic ya no la creamos*/
...
/*Se genera vector de diferencias*/

gen dif= saludh
by bc_correlat: egen asig_declaradas= sum(g257)
replace dif= dif+(asig_declaradas-monto_afam_pe)
replace dif=dif+(asig_declaradas-monto_afam_con) if g256==1
g aux= abs(ht11-bc_ht11_sss_corr)
g otra_dif=0
replace otra_dif=1 if (abs(dif/aux)/aux)>0.1 & bc_nper==1
replace otra_dif=0 if ht11==bc_ht11_sss_corr
