...
rename ht11_iecon bc_ht11_iecon
bysort bc_correlat: gen max_nper = _N
bysort bc_correlat: gen bc_percap_iecon = bc_ht11_sss/max_nper

foreach var in bc_ht11_sss bc_ht11_css bc_percap_iecon bc_ht11_sss_corr bc_salud {
...
append using "$rutainterm/servdom11", nol
sort bc_correlat bc_nper

*-------------------------------------------------------------------------------
*- Se retoman etiquetas originales de varibles de ingreso de INE 
*- que fueron previamente cambiadas en el paso 2.

rename ine_ytdop ytdop
rename ine_ytdos ytdos
rename ine_ytinde ytinde
rename ine_ytransf ytransf
rename ine_pt1 pt1
rename ine_pt2 pt2
rename ine_pt4 pt4
